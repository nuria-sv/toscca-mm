...
#' @export
myHeatmap <- function(mat, palette_values = tosccamm:::mpalette, blue = NULL, xlab = "", ylab = "",
                      show_axes = TRUE, show_labels = TRUE, K = NULL) {

  if(is.null(palette_values) | all(palette_values %in% tosccamm:::mpalette)) blue = TRUE
  ...
  K = ifelse(is.matrix(mat), 1, length(mat))
  ...
  if(isTRUE(K==1)) {
    ...
    cols <- colorRampPalette(palette_values[bor])(length(unique(mat_df)))
...
#' @export
plt.selstab=
  function (object, X, Y, palette_values = tosccamm:::mpalette, blue = T, mm = T, k=1)
  {
    ...
    if(isTRUE(blue)) bor = (length(palette_values)/2):1
    if(isFALSE(blue)) bor = (length(palette_values)/2):(length(palette_values))
    custom_colors <- colorRampPalette(palette_values[bor])(n_colors) # viridis::viridis(n_colors + 1, option = palette)[-1]
