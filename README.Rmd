...
Zi2 = (sin(100*times))^times +   times * 0.65 +rnorm(1,0,0.75)
Zi = cbind(Zi1, Zi2)
...
shed_blue_60 <- viridis::magma(5)[4] #"#6864fc"
...
plot1 <- ggplot(lv1, aes(x = time, y = scale(K1), group = id)) +
  ...
  scale_color_manual(values = c("Estimated" = "black", "True" = viridis::magma(5)[3], "Smoothed" = shed_blue_60)) +  # Specify color manually
...
plot3 <- ggplot(lv3, aes(x = time, y = scale(K3), group = id)) +
  geom_line(alpha = 0.1, aes(color = "Estimated", linetype = "Estimated", linewidth = "Estimated")) +  # Estimated (black)
  geom_hline(yintercept =0, color = viridis::magma(5)[3], linetype = 1, linewidth = 1.5) +
...
shared_legend <- ggplot(lv1, aes(x = 1, y = 0, group = id)) +
  ...
  scale_color_manual(values = c("Estimated" = "black", "True" = viridis::magma(5)[3], "Mean" = shed_blue_60)) +
...
plotTrue_alpha1 = ggplot(df, aes(x = Index, y = Value, color = Category)) +
  geom_point(size = 3) +  # Use points with a size of 3 (adjust if needed)
  scale_color_manual(values = c("Zero" = scales::alpha("black", 0.01), "Non-zero" = viridis::magma(5)[3])) +  # Define colors
...
for (k in 1:K) {
    ...
    plt = plt  + ggtitle("cc w.r.t. sparsity levels") + theme(plot.title = element_text(size = 16, face = "bold", color = "black",
                                                                                        hjust = 0.5, vjust = 1.5,
                                                                                        lineheight = 1.2))
    print(plt)
    }
