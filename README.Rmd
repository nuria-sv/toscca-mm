...
```{r palette, fig.height=2, fig.width=14}
...
shed_blue_60 <- tosccamm:::mpalette[3] # viridis::magma(5)[4] #"#6864fc"
...
plot1 <- ggplot(lv1, aes(x = time, y = scale(K1), group = id)) +
  ...
  scale_color_manual(values = c("Estimated" = tosccamm:::mpalette[1], "True" = tosccamm:::mpalette[6], "Smoothed" = shed_blue_60)) +  # Specify color manually
...
plot3 <- ggplot(lv3, aes(x = time, y = scale(K3), group = id)) +
  geom_line(alpha = 0.1, aes(color = "Estimated", linetype = "Estimated", linewidth = "Estimated")) +  # Estimated (black)
  geom_hline(yintercept =0, color = tosccamm:::mpalette[6], linetype = 1, linewidth = 1.5) +
...
shared_legend <- ggplot(lv1, aes(x = 1, y = 0, group = id)) +
  ...
  scale_color_manual(values = c("Estimated" = tosccamm:::mpalette[1], "True" = tosccamm:::mpalette[6], "Mean" = shed_blue_60)) +
...
# shed_blue_60 = viridis::magma(5)[4]
...
plot_alpha1 = ggplot(df, aes(x = Index, y = Value, color = Category)) +
  ...
  scale_color_manual(values = c("Zero" = scales::alpha(tosccamm:::mpalette[1], 0.01), "Non-zero" = shed_blue_60)) +  # Define colors
...
plotTrue_alpha1 = ggplot(df, aes(x = Index, y = Value, color = Category)) +
  geom_point(size = 3) +  # Use points with a size of 3 (adjust if needed)
  scale_color_manual(values = c("Zero" = scales::alpha(tosccamm:::mpalette[1], 0.01), "Non-zero" = tosccamm:::mpalette[6])) +  # Define colors
...
```{r plotNoise, echo=FALSE, warning=FALSE, message=FALSE}
grid.arrange(
  gridExtra::arrangeGrob(
...
```{r gridPlots, echo=FALSE}
# for (k in 1:K) {
#     x.k = res_k[[k]]
#     plt = myHeatmap(x.k$canCor_grid, show_labels = T, K, palette)
#     plt = plt  + ggtitle("cc w.r.t. sparsity levels") + theme(plot.title = element_text(size = 16, face = "bold", color = tosccamm:::mpalette[1],
#                                                                                         hjust = 0.5, vjust = 1.5,
#                                                                                         lineheight = 1.2))
#     print(plt)
#     }

myHeatmap(res_k, show_labels = T)
```

```{r permutationTesting, echo=F}
res_k = list()
...
for (k in 1:5) {
  if(k > 1) {
    ...
    nonz_a = as.numeric(table(res_k[[k-1]]$alpha != 0)[2])
    nonz_b = as.numeric(table(res_k[[k-1]]$beta != 0)[2])
  }
...
K = length(res_k)
perm_cc = toscamm.perm(XX2, YY2, nonzero_a = sapply(1:K, function(k)res_k[[k]]$nonzero_a), nonzero_b = sapply(1:K, function(k)res_k[[k]]$nonzero_b), K, alpha_init = "uniform", folds = 1, toPlot = T, draws = 1000, model = "lme", 
                       lmeformula = " ~ 0 + poly(time,3) + (1|id)",
                           cancor=sapply(1:K, function(k)res_k[[k]]$cancor), bootCCA = NULL, silent = TRUE, parallel_logic = TRUE,
                         nuisanceVar = 0, testStatType = "CC")
